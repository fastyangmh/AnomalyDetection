...
class PredictGUI(BasePredictGUI):
    ...
    def display_output(self, fake_sample):
        ...
        cmap = 'gray' if sample.shape[0] == 1 else None
        ...
        for idx in range(1, rows * cols + 1):
            ...
            if (idx - 1) % 3 == 0:
                # plot real
                subplot.imshow(sample[(idx - 1) // cols], cmap=cmap)
            elif (idx - 1) % 3 == 1:
                # plot fake
                subplot.imshow(fake_sample[(idx - 1) // cols], cmap=cmap)
            elif (idx - 1) % 3 == 2:
                # plot diff
                subplot.imshow(diff[(idx - 1) // cols], cmap=cmap)
            subplot.axis('off')
        self.image_canvas.draw()
